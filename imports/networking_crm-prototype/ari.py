...
def _normalize_legacy_argv(argv: Optional[List[str]]) -> List[str]:
    args = list(argv or [])
    if not args:
        return args
...
def _add_networking_parsers(subparsers: argparse._SubParsersAction) -> None:
    ...
    networking_subparsers = networking_parser.add_subparsers(dest="networking_command", required=True)
    ...
    networking_subparsers.add_parser("init-db", help="Create the networking module SQLite database.")
    ...
    contacts_note_parser = contacts_subparsers.add_parser("add-note", help="Attach a note to a contact.")
    contacts_note_parser.add_argument("--contact-id", type=int, required=True, help="Existing contact id.")
    ...
    followups_add_parser = followups_subparsers.add_parser("add", help="Add a follow-up for a contact.")
    followups_add_parser.add_argument("--contact-id", type=int, required=True, help="Existing contact id.")
    followups_add_parser.add_argument("--due-on", required=True, help="Due date in YYYY-MM-DD format.")
    followups_add_parser.add_argument("--reason", help="Reason or context for the follow-up.")
    followups_complete_parser = followups_subparsers.add_parser("complete", help="Mark a follow-up as completed.")
    followups_complete_parser.add_argument("--id", type=int, required=True, help="Existing follow-up id.")
...
def _add_docs_parsers(subparsers: argparse._SubParsersAction) -> None:
    ...
    linkedin_parser = content_subparsers.add_parser("linkedin", help="Generate a LinkedIn-ready draft.")
    linkedin_parser.add_argument("--topic", required=True, help="Topic or project thread to draft about.")
    linkedin_parser.add_argument("--style", choices=["story", "tactical", "insight"], help="Drafting style.")
    linkedin_parser.add_argument("--format", choices=["full", "short"], default="full", help="Draft render mode.")
    linkedin_parser.add_argument("--save", action="store_true", help="Save the generated draft under ~/ARI/content/.")
    ...
    mode_group.add_argument("--detailed", action="store_true", help="Generate a more reflective draft.")
    ...
    short_video_parser = script_subparsers.add_parser("short-video", help="Generate a short-form video script.")
    short_video_parser.add_argument("--topic", required=True, help="Topic or project thread to script about.")
    short_video_parser.add_argument("--style", choices=["story", "tactical", "insight"], help="Scripting style.")
    ...
    terminal_parser = demo_subparsers.add_parser("terminal", help="Capture a terminal command for demo reuse.")
    terminal_parser.add_argument("--command", dest="shell_command", required=True, help="Shell command string to run and capture.")
    terminal_parser.add_argument("--save-name", help="Optional artifact name.")
    record_parser = demo_subparsers.add_parser("record", help="Run a command while capturing a screen recording.")
    record_parser.add_argument("--command", dest="shell_command", required=True, help="Shell command string to run while recording.")
    record_parser.add_argument("--save-name", help="Optional artifact name.")
    record_parser.add_argument("--pre-delay", type=float, default=1.0, help="Seconds to wait before executing the command.")
    record_parser.add_argument("--post-delay", type=float, default=2.0, help="Seconds to wait after the command finishes.")
    ...
    video_build_parser = video_subparsers.add_parser("build", help="Record a command execution into an ARI video artifact.")
    video_build_parser.add_argument("--command", dest="shell_command", required=True, help="Shell command to run while recording.")
    video_build_parser.add_argument("--save-name", help="Optional artifact name.")
    video_build_parser.add_argument("--pre-delay", type=float, default=1.0, help="Seconds to wait before executing the command.")
    video_build_parser.add_argument("--post-delay", type=float, default=2.0, help="Seconds to wait after the command finishes.")
    ...
    clip_build_parser = clip_subparsers.add_parser("build", help="Trim an existing ARI video into a reusable clip.")
    clip_build_parser.add_argument("--video", required=True, help="Path to an existing raw .mov artifact created by ARI.")
    clip_build_parser.add_argument("--save-name", help="Optional artifact name.")
    clip_build_parser.add_argument("--mode", choices=["default", "proof"], default="default", help="Standard trim or proof-aware trim.")
    clip_build_parser.add_argument("--trim-start", type=float, default=0.5, help="Seconds to trim from the start of the source video.")
    clip_build_parser.add_argument("--trim-end", type=float, default=0.5, help="Seconds to trim from the end of the source video.")
    ...
    frame_build_parser = frame_subparsers.add_parser("build", help="Crop an existing ARI video into a framed artifact.")
    frame_build_parser.add_argument("--video", required=True, help="Path to an existing raw or clipped .mov artifact created by ARI.")
    frame_build_parser.add_argument("--save-name", help="Optional artifact name.")
    frame_build_parser.add_argument("--mode", choices=["center", "terminal", "vertical"], default="terminal", help="Crop mode.")
    frame_build_parser.add_argument("--anchor", choices=["auto", "frontmost", "terminal", "browser"], default="auto", help="Window anchor mode.")
    frame_build_parser.add_argument("--window-title", help="Optional partial window-title match.")
    frame_build_parser.add_argument("--padding", type=int, default=40, help="Padding around the detected window.")
    frame_build_parser.add_argument("--width", type=int, default=800, help="Crop width for center and terminal modes.")
    frame_build_parser.add_argument("--height", type=int, default=600, help="Crop height for center and terminal modes.")

    storyboard_parser = docs_subparsers.add_parser("storyboard", help="Storyboard planning commands.")
    storyboard_subparsers = storyboard_parser.add_subparsers(dest="storyboard_command", required=True)
    short_video_storyboard_parser = storyboard_subparsers.add_parser("short-video", help="Generate a short-form video storyboard.")
    short_video_storyboard_parser.add_argument("--topic", required=True, help="Topic or project thread to plan around.")
    short_video_storyboard_parser.add_argument("--style", choices=["story", "tactical", "insight"], help="Storyboard style.")
    short_video_storyboard_parser.add_argument("--demo-file", help="Optional saved demo artifact path from `ari docs demo terminal`.")
    short_video_storyboard_parser.add_argument("--save", action="store_true", help="Save the generated storyboard under ~/ARI/storyboards/.")
    ...
    record_plan = record_plan_subparsers.add_parser("plan", help="Generate a short-form recording plan.")
    record_plan.add_argument("--topic", required=True, help="Topic or project thread to plan around.")
    record_plan.add_argument("--style", choices=["story", "tactical", "insight"], help="Recording plan style.")
    record_plan.add_argument("--demo-file", help="Optional saved demo artifact path from `ari docs demo terminal`.")
    record_plan.add_argument("--save", action="store_true", help="Save the generated plan under ~/ARI/recordings/.")

    session_parser = docs_subparsers.add_parser("session", help="Session documentation workflow commands.")
    session_subparsers = session_parser.add_subparsers(dest="session_command", required=True)
    session_build_parser = session_subparsers.add_parser("build", help="Build a structured session file from recent local demo activity.")
    session_build_parser.add_argument("--limit", type=int, default=5, help="Number of recent demo files to aggregate.")
...
def _add_api_parsers(subparsers: argparse._SubParsersAction) -> None:
    ...
    memory_parser = api_subparsers.add_parser("memory", help="Canonical ARI structured memory API commands.")
    memory_subparsers = memory_parser.add_subparsers(dest="api_memory_command", required=True)

    memory_remember_parser = memory_subparsers.add_parser("remember", help="Remember canonical ARI structured memory.")
    ...
    memory_remember_parser.add_argument("--tags-json", dest="tags_json", default="[]", help="JSON array of memory tags.")
    memory_remember_parser.add_argument("--json", dest="as_json", action="store_true", help="Render JSON output.")

    memory_list_parser = memory_subparsers.add_parser("list", help="List canonical ARI structured memory.")
    memory_list_parser.add_argument("--type", action="append", default=[], help="Structured memory type filter. Repeatable.")
    memory_list_parser.add_argument("--limit", type=int, default=20, help="Maximum memories to return.")
    memory_list_parser.add_argument("--json", dest="as_json", action="store_true", help="Render JSON output.")

    memory_search_parser = memory_subparsers.add_parser("search", help="Search canonical ARI structured memory.")
    memory_search_parser.add_argument("--query", default="", help="Query text.")
    memory_search_parser.add_argument("--type", action="append", default=[], help="Structured memory type filter. Repeatable.")
    memory_search_parser.add_argument("--limit", type=int, default=20, help="Maximum memories to return.")
    memory_search_parser.add_argument("--json", dest="as_json", action="store_true", help="Render JSON output.")

    memory_get_parser = memory_subparsers.add_parser("get", help="Get a canonical ARI structured memory record by id.")
    memory_get_parser.add_argument("--id", required=True, help="Memory id.")
    memory_get_parser.add_argument("--json", dest="as_json", action="store_true", help="Render JSON output.")

    coordination_parser = api_subparsers.add_parser("coordination", help="Canonical ARI coordination runtime commands.")
    coordination_subparsers = coordination_parser.add_subparsers(dest="api_coordination_command", required=True)

    coordination_put_parser = coordination_subparsers.add_parser("put", help="Upsert canonical coordination state.")
    coordination_put_parser.add_argument("--entity", required=True, help="Coordination entity name.")
    coordination_put_parser.add_argument("--payload-json", required=True, help="JSON object payload.")

    coordination_get_parser = coordination_subparsers.add_parser("get", help="Get canonical coordination state by id.")
    coordination_get_parser.add_argument("--entity", required=True, help="Coordination entity name.")
    coordination_get_parser.add_argument("--id", required=True, help="Record id.")

    coordination_list_parser = coordination_subparsers.add_parser("list", help="List canonical coordination state.")
    coordination_list_parser.add_argument("--entity", required=True, help="Coordination entity name.")
    coordination_list_parser.add_argument("--limit", type=int, default=50, help="Maximum records to return.")

    policy_parser = api_subparsers.add_parser("policy", help="Canonical ARI policy engine commands.")
    policy_subparsers = policy_parser.add_subparsers(dest="api_policy_command", required=True)

    policy_awareness_parser = policy_subparsers.add_parser("awareness", help="Canonical awareness derivation and persistence.")
    policy_awareness_subparsers = policy_awareness_parser.add_subparsers(dest="api_policy_awareness_command", required=True)
    policy_awareness_derive = policy_awareness_subparsers.add_parser("derive", help="Derive an awareness snapshot.")
    policy_awareness_derive.add_argument("--payload-json", required=True, help="JSON object payload.")
    policy_awareness_store = policy_awareness_subparsers.add_parser("store", help="Store an awareness snapshot.")
    policy_awareness_store.add_argument("--payload-json", required=True, help="JSON object payload.")
    policy_awareness_subparsers.add_parser("latest", help="Get the latest canonical awareness snapshot.")

    policy_orchestration_parser = policy_subparsers.add_parser("orchestration-classify", help="Classify builder output canonically.")
    policy_orchestration_parser.add_argument("--payload-json", required=True, help="JSON object payload.")

    policy_improvement_parser = policy_subparsers.add_parser("improvements", help="Canonical self-improvement policy commands.")
    policy_improvement_subparsers = policy_improvement_parser.add_subparsers(dest="api_policy_improvement_command", required=True)
    policy_improvement_detect = policy_improvement_subparsers.add_parser("detect", help="Detect ranked capability gaps.")
    policy_improvement_detect.add_argument("--payload-json", required=True, help="JSON object payload.")
    policy_improvement_subparsers.add_parser("focus", help="Get the top self-improvement focus.")

    policy_project_parser = policy_subparsers.add_parser("project", help="Canonical project-planning policy commands.")
    policy_project_subparsers = policy_project_parser.add_subparsers(dest="api_policy_project_command", required=True)
    policy_project_draft = policy_project_subparsers.add_parser("draft", help="Build a canonical project draft.")
    policy_project_draft.add_argument("--payload-json", required=True, help="JSON object payload.")
    policy_project_subparsers.add_parser("focus", help="Sync and return the canonical project focus.")
    ...
    notes_save_parser.add_argument("--json", dest="as_json", action="store_true", help="Render JSON output.")
    ...
    notes_search_parser.add_argument("--limit", type=int, default=10, help="Maximum notes to return.")
    notes_search_parser.add_argument("--json", dest="as_json", action="store_true", help="Render JSON output.")
    ...
    tasks_create_parser.add_argument("--json", dest="as_json", action="store_true", help="Render JSON output.")
    ...
    tasks_list_parser.add_argument("--json", dest="as_json", action="store_true", help="Render JSON output.")
    ...
    tasks_get_parser.add_argument("--json", dest="as_json", action="store_true", help="Render JSON output.")
...
def _add_legacy_alias_parsers(subparsers: argparse._SubParsersAction) -> None:
    ...
    contacts_note_parser.add_argument("--contact-id", type=int, required=True, help=argparse.SUPPRESS)
    ...
    followups_add_parser.add_argument("--contact-id", type=int, required=True, help=argparse.SUPPRESS)
    ...
    linkedin_parser.add_argument("--style", choices=["story", "tactical", "insight"], help=argparse.SUPPRESS)
    linkedin_parser.add_argument("--format", choices=["full", "short"], default="full", help=argparse.SUPPRESS)
    ...
    short_video_parser.add_argument("--style", choices=["story", "tactical", "insight"], help=argparse.SUPPRESS)
    ...
    terminal_parser.add_argument("--command", dest="shell_command", required=True, help=argparse.SUPPRESS)
    ...
    record_parser.add_argument("--command", dest="shell_command", required=True, help=argparse.SUPPRESS)
    ...
    video_build_parser.add_argument("--command", dest="shell_command", required=True, help=argparse.SUPPRESS)
    ...
    clip_build_parser.add_argument("--mode", choices=["default", "proof"], default="default", help=argparse.SUPPRESS)
    ...
    frame_build_parser.add_argument("--mode", choices=["center", "terminal", "vertical"], default="terminal", help=argparse.SUPPRESS)
    frame_build_parser.add_argument("--anchor", choices=["auto", "frontmost", "terminal", "browser"], default="auto", help=argparse.SUPPRESS)
    ...
    storyboard_subparsers = storyboard_parser.add_subparsers(dest="storyboard_command", required=True)
    short_video_storyboard_parser = storyboard_subparsers.add_parser("short-video", help=argparse.SUPPRESS)
    short_video_storyboard_parser.add_argument("--topic", required=True, help=argparse.SUPPRESS)
    short_video_storyboard_parser.add_argument("--style", choices=["story", "tactical", "insight"], help=argparse.SUPPRESS)
    short_video_storyboard_parser.add_argument("--demo-file", help=argparse.SUPPRESS)
    short_video_storyboard_parser.add_argument("--save", action="store_true", help=argparse.SUPPRESS)
    ...
    record_plan.add_argument("--style", choices=["story", "tactical", "insight"], help=argparse.SUPPRESS)
...
def main(argv: Optional[List[str]] = None, db_path: Path = DB_PATH) -> int:
    ...
    if args.command == "api":
        ...
        if args.api_command == "policy" and args.api_policy_command == "awareness" and args.api_policy_awareness_command == "derive":
            return handle_api_policy_awareness_derive(args, db_path=db_path)
        if args.api_command == "policy" and args.api_policy_command == "awareness" and args.api_policy_awareness_command == "store":
            return handle_api_policy_awareness_store(args, db_path=db_path)
        if args.api_command == "policy" and args.api_policy_command == "awareness" and args.api_policy_awareness_command == "latest":
        ...
        if args.api_command == "policy" and args.api_policy_command == "improvements" and args.api_policy_improvement_command == "detect":
            return handle_api_policy_improvement_detect(args, db_path=db_path)
        if args.api_command == "policy" and args.api_policy_command == "improvements" and args.api_policy_improvement_command == "focus":
            return handle_api_policy_improvement_focus(args, db_path=db_path)
        if args.api_command == "policy" and args.api_policy_command == "project" and args.api_policy_project_command == "draft":
            return handle_api_policy_project_draft(args, db_path=db_path)
        if args.api_command == "policy" and args.api_policy_command == "project" and args.api_policy_project_command == "focus":
